...
## Сессия 3: 2026-04-23 - Усовершенствование сканирования

### Что сделано

| Коммит | Описание |
|--------|----------|
| `91bcedf` | feat: усовершенствование логики сканирования хранилищ |
| `fa23ace` | docs: обновить документацию - фильтры, новые поступления, логика сканирования |

### Реализованные фичи (Сессия 3)
...
- **Обновление API** - параметр `availability` в `/api/books` и `/api/search`
- **Новые поля БД** - `is_available`, `is_new_arrival`, `last_seen`

### Изменённые файлы

| Файл | Изменения |
|------|-----------|
| `app/database.py` | + поля `is_available`, `is_new_arrival`, `last_seen`; функции `reset_stale_new_arrivals()`, `upsert_book_preserve()` |
| `app/models.py` | + поля `is_new_arrival`, `last_seen` в `BookResponse` |
| `app/routers/books.py` | + параметр `availability` |
| `app/routers/search.py` | + параметр `availability` |
| `app/routers/sources.py` | обновлён ответ `/scan` |
| `services/importer.py` | `reset_stale_new_arrivals` при сканировании, логика новых поступлений |
| `web/index.html` | фильтр `availabilityFilter`, CSS классы, JS логика |
| `reset_db.py` | обновлена схема |
| `AGENTS.md` | обновлена документация |
| `README.md` | обновлена документация |
| `ARCHITECTURE.md` | обновлена архитектура |

### Логика сканирования (новая)

```
1. Сброс устаревших флагов is_new_arrival (старше 7 дней)
2. Сканирование файловой системы
3. Для каждого файла:
   - Существует в БД → подтверждение наличия (is_available=1, last_seen)
   - Новый файл → добавление с is_new_arrival=1
4. Отсутствующие книги → is_available=0
```
